...
void vmx_write_encls_bitmap(struct kvm_vcpu *vcpu, struct vmcs12 *vmcs12)
{
	...
	if (guest_cpuid_has(vcpu, X86_FEATURE_SGX) &&
		 ...
		 * The MSRs are not loaded/saved on VM-Enter/VM-Exit as they
		 * aren't allowed in the hardware-supported lists and writing
...
static __init int hardware_setup(void)
{
	 ...
	 * Launch Control is supported but disabled, i.e. the hash MSRs
	 * exist but are locked, then we have to use the current MSR
	 * values as the MSRs are not writable.
	 */
#ifdef CONFIG_INTEL_SGX_CORE
	if (boot_cpu_has(X86_FEATURE_SGX_LC) ||
